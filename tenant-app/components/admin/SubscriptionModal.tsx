...
export function SubscriptionModal({
  ...
  return (
    <>
          ...
          <div className="border border-gray-200 rounded-3xl bg-white p-6 shadow-sm">
            ...
            {activeTab === "CARD" ? (
              <form onSubmit={handleCardPayment} className="space-y-4">
                ...
                <div className="text-[11px] text-gray-400 text-center bg-gray-50 p-2.5 rounded-xl border border-gray-100 leading-relaxed">
                  בלחיצה על כפתור התשלום הינך מאשר/ת אוטומטית את 
                  <button type="button" onClick={() => setShowTerms(true)} className="text-indigo-600 underline font-bold mx-1 hover:text-indigo-800">תנאי השימוש והסכם הרישיון</button>
                  (מערכת ענן אוטונומית, מחיקת תמונות אוטומטית לאחר 15 יום).
                </div>

                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl font-black text-lg shadow-md transition-all mt-2"
                >
              ...
              <form onSubmit={handleSalesSubmit} className="space-y-4">
                ...
                <div className="text-[11px] text-gray-400 text-center bg-gray-50 p-2.5 rounded-xl border border-gray-100 leading-relaxed">
                  בשליחת הבקשה הינך מאשר/ת את 
                  <button type="button" onClick={() => setShowTerms(true)} className="text-indigo-600 underline font-bold mx-1 hover:text-indigo-800">תנאי השימוש והסכם הרישיון של המערכת</button>.
                </div>
      ...
      {/* Embedded Terms & Licensing Modal */}
      {showTerms && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 md:p-8 shadow-2xl flex flex-col max-h-[90vh] text-gray-800" dir="rtl">
            <div className="flex justify-between items-center border-b pb-4 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900">תנאי שימוש והסכם רישיון למערכת HCL</h3>
                  <p className="text-xs text-gray-500">Software as a Service (SaaS) Agreement</p>
                </div>
              </div>
              <button onClick={() => setShowTerms(false)} className="text-gray-400 hover:text-gray-600 text-2xl transition"><i className="fas fa-times"></i></button>
            </div>

            <div className="flex-grow overflow-y-auto space-y-4 text-right text-gray-700 text-xs leading-relaxed pr-2">
              <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 text-indigo-900">
                <strong>הסכמה אוטומטית:</strong> שליחת טופס, הרשמה לניסיון, כניסה או ביצוע תשלום מהווים הסכמה מלאה, סופית ומחייבת לכלל תנאי הסכם זה.
              </div>

              <div>
                <h4 className="font-bold text-indigo-900 text-sm mb-1">1. מהות השירות (SaaS)</h4>
                <p>מערכת HCL פועלת כשירות ענן בתשלום ומספקת כלים מתקדמים לניהול, תיאום ותיעוד אחזקת מבנים, ניקיון וניהול צוותי שטח.</p>
              </div>

              <div>
                <h4 className="font-bold text-indigo-900 text-sm mb-1">2. תשתית ענן עצמאית ואוטונומיה מוחלטת</h4>
                <p>המערכת פועלת באופן בלעדי על גבי שרתי אינטרנט וענן חיצוניים ומאובטחים. <strong>המערכת הינה עצמאית ואוטונומית לחלוטין ואינה מקושרת לרשתות, למחשבים או למאגרי המידע הפנימיים של הארגון.</strong> המערכת אינה שואבת ואינה עושה שימוש בשום נתון פנימי של הלקוח.</p>
              </div>

              <div>
                <h4 className="font-bold text-indigo-900 text-sm mb-1">3. מדיניות מחיקת תמונות ומדיה לאחר 15 יום</h4>
                <p>כל התמונות המועלות לצורך דיווח וטיפול בתקלות מאוחסנות בשרת מדיה חיצוני מאובטח. <strong>כל התמונות נמחקות באופן אוטומטי ולצמיתות לאחר 15 ימים ממועד העלאתן, ללא יכולת שחזור</strong>, במטרה לייעל משאבים ולחסוך מקום אחסון בשרתים.</p>
              </div>

              <div>
                <h4 className="font-bold text-indigo-900 text-sm mb-1">4. אבטחת מידע ופרטיות</h4>
                <p>הספק נוקט באמצעי אבטחה והצפנה מקובלים (SSL/TLS). המידע נשמר לשימושו הבלעדי של הלקוח ואינו מועבר לצדדים שלישיים.</p>
              </div>
            </div>

            <div className="border-t pt-4 mt-4 flex justify-end">
              <Button onClick={() => setShowTerms(false)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm">
                סגור ואישור
              </Button>
            </div>
          </div>
