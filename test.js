        ...
        function openWorkersModal() { document.getElementById('workersModal').classList.remove('hidden'); renderWorkersListUI(); }
        ...
        function addWorkerToList() {
            ...
            workersPool.push(name); inp.value = ''; renderWorkersListUI();
        }
        ...
        function resetPrintStamp(realIdx) {
            ...
            showCustomConfirm("ביטול סטטוס", "לבטל את סטטוס 'בעבודה' למשימה זו?", async () => {
                ...
                try {
                    await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: "UNMARK_PRINTED", tasks: [{ sheet: task.sheet, room: task.room, defect: task.defect, comment: task.comment || "", date: task.dateStr }] }) });
        ...
        async function revertWorkerTasks() {
            ...
            const workerTasks = allTasks.filter(t => t.status === "בעבודה" && t.printedWorker === workerName);
        ...
        function moveTask(realIdx, tgtSheet) {
            ...
            showCustomConfirm("העברת משימה", "לשים לב: להעביר משימה זו לצוות אחר?", async () => {
                showLoader("מעביר...");
                try {
                    const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: "MOVE_TASK", sourceSheet: task.sheet, targetSheet: tgtSheet, room: task.room, defect: task.defect, comment: task.comment || "" }) });
                    const out = await res.json();
                    if(out.status === "success") { allTasks = allTasks.filter(t => t !== task); if(fpInstance) fpInstance.redraw(); renderTasks(); showToast("המשימה הועברה בהצלחה", "success"); }
                } catch(e) { console.error(e); } hideLoader();
            }, "fa-exchange-alt");
        }
            ...
            if(fDept !== "ALL") filtered = filtered.filter(t => t.dept === fDept);
              ...
              if(searchQuery) {
                  filtered = filtered.filter(t => 
                      ...
                      (t.printedWorker && t.printedWorker.toLowerCase().includes(searchQuery))
                  );
              }
            ...
            filtered.forEach((task) => {
                ...
                if (viewMode === 'table') {
                    tableHTML += `<tr id="task-card-${realIdx}" class="hover:bg-gray-50 transition-colors ${isPrinted ? 'bg-orange-50/30' : (ageClass ? ageClass.split(' ')[1] : '')}">
                        ...
                        <td class="p-3 font-bold text-xs text-orange-900">${task.printedWorker || ''}</td>
                    ...
                    <div class="flex flex-row md:flex-col items-center justify-center gap-2 w-full md:w-48">
                        ${isPrinted ? `
                            <div class="flex flex-col items-center justify-center gap-1 bg-orange-100 border border-orange-300 rounded-xl px-2 py-1 text-xs text-orange-900 w-full text-center shadow-inner">
                                ...
                                ${task.printedWorker ? `<span class="font-bold whitespace-nowrap overflow-hidden text-ellipsis max-w-full" title="${task.printedWorker}">${task.printedWorker}</span>` : ''}
                ...
                tbody.innerHTML += `<tr><td>${task.dateStr}</td><td>${task.inspector || "אנדרי"}</td><td>${task.dept}</td><td>${task.room}</td><td>${task.defect}</td><td>${task.comment || ""}</td><td>${isPrinted ? "בעבודה" : "פתוח"}</td><td>${task.printedWorker || ""}</td></tr>`;
            });
